...
export default function InvestmentDetail({ investmentId }: { investmentId: string }) {
  ...
  if (!investment) {
    return (
      <div className="text-center py-12">
        <p className="text-slate-500 mb-4">Investment not found.</p>
  ...
  return (
    <div>
      {/* Back link */}
      <Link href="/" className="text-sm text-slate-500 hover:text-slate-700 mb-4 inline-flex items-center gap-1">
      ...
      <div className="flex items-start justify-between mt-2 mb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-bold text-slate-800">{investment.name}</h1>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm mb-6">
        <div className="flex items-center justify-between px-6 py-3 border-b border-slate-100">
          <h3 className="font-semibold text-slate-700">Price Chart</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-5">
          <h3 className="font-semibold text-slate-700 mb-4">Your Holdings</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-sm text-slate-500">Shares/Coins</span>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-800">{formatCurrency(investment.avgCost)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-slate-500">Cost Basis</span>
              <span className="font-medium text-slate-800">{formatCurrency(costBasis)}</span>
            </div>
            <hr className="border-slate-100" />
            <div className="flex justify-between">
              <span className="text-sm text-slate-500">Market Value</span>
              <span className="font-bold text-slate-800">{formatCurrency(marketValue)}</span>
            </div>
            <div className="flex justify-between">
        ...
        {/* Market Stats */}
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-5">
          <h3 className="font-semibold text-slate-700 mb-4">Market Stats</h3>
          {detail && (
            <div className="space-y-3">
              {detail.open > 0 && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Open</span>
                  <span className="font-medium text-slate-800">{formatCurrency(detail.open)}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-sm text-slate-500">Day Range</span>
                <span className="font-medium text-slate-800">
                  {formatCurrency(detail.dayLow)} - {formatCurrency(detail.dayHigh)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-slate-500">52-Week Range</span>
                <span className="font-medium text-slate-800">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-slate-800">{formatVolume(detail.volume)}</span>
              </div>
              {detail.avgVolume > 0 && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Avg Volume</span>
                  <span className="font-medium text-slate-800">{formatVolume(detail.avgVolume)}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-sm text-slate-500">Market Cap</span>
                <span className="font-medium text-slate-800">{formatLargeNumber(detail.marketCap)}</span>
              </div>
            </div>
          )}
        </div>
          ...
          {detail && investment.type === "STOCK" && (
            <div className="space-y-3">
              {detail.trailingPE != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">P/E (TTM)</span>
                  <span className="font-medium text-slate-800">{detail.trailingPE.toFixed(2)}</span>
                </div>
              )}
              {detail.forwardPE != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Forward P/E</span>
                  <span className="font-medium text-slate-800">{detail.forwardPE.toFixed(2)}</span>
                </div>
              )}
              {detail.eps != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">EPS (TTM)</span>
                  <span className="font-medium text-slate-800">{formatCurrency(detail.eps)}</span>
                </div>
              )}
              {detail.dividendYield != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Dividend Yield</span>
                  <span className="font-medium text-slate-800">{(detail.dividendYield * 100).toFixed(2)}%</span>
                </div>
              )}
              {detail.beta != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Beta</span>
                  <span className="font-medium text-slate-800">{detail.beta.toFixed(2)}</span>
                </div>
              )}
              {detail.profitMargins != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Profit Margin</span>
                  <span className="font-medium text-slate-800">{(detail.profitMargins * 100).toFixed(1)}%</span>
                </div>
              )}
              ...
              {detail.targetMeanPrice != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Analyst Target</span>
                  <span className="font-medium text-slate-800">{formatCurrency(detail.targetMeanPrice)}</span>
                </div>
              )}
          ...
          {detail && investment.type === "CRYPTO" && (
            <div className="space-y-3">
              ...
              {detail.athChangePercent != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">From ATH</span>
              ...
              {detail.priceChange7d != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">7d Change</span>
              ...
              {detail.priceChange30d != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">30d Change</span>
              ...
              {detail.priceChange1y != null && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">1y Change</span>
