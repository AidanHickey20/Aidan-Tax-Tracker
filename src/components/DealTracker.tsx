    ...
    <div>
      ...
      {openDeals.length === 0 && closedDeals.length === 0 ? (
        ...
        <>
          ...
          {/* Past Deals — filed away in an archive drawer */}
          {closedDeals.length > 0 && (
            <div className="mt-10">
              <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/40">
                ...
                >
                  {/* Folder icon */}
                  <svg className="w-5 h-5 flex-shrink-0 text-slate-500 group-hover:text-slate-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-slate-300">Filed — Past Deals</span>
                      <span className="text-xs px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">{closedDeals.length}</span>
                    </div>
                    <span className="text-xs text-slate-500">
                      Closed & archived · Total profit{" "}
                      <span className="text-emerald-500 font-medium">
                        {formatCurrency(closedDeals.reduce((s, d) => s + (d.closedProfit || 0), 0))}
                      </span>
                    </span>
                  </div>
                  <svg
                    className={`w-4 h-4 flex-shrink-0 text-slate-500 transition-transform ${showClosed ? "rotate-90" : ""}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
