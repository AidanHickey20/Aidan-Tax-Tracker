...
export default function ExportPage() {
  ...
  if (loading) {
    return <div className="text-slate-400 py-8">Loading...</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="print:hidden flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-slate-800">Accountant Export</h2>
        <button
          onClick={handlePrint}
      ...
      {/* Printable Document */}
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm print:border-0 print:shadow-none print:rounded-none">
        {/* Header */}
        <div className="px-8 py-6 border-b border-slate-200 print:border-b-2 print:border-slate-300">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Annual Tax Summary</h1>
              <p className="text-sm text-slate-500 mt-1">Self-Employed Real Estate Professional</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-semibold text-slate-800">{year} Tax Year</p>
              <p className="text-xs text-slate-400 mt-1">{firstDate} - {lastDate}</p>
              <p className="text-xs text-slate-400">{entries.length} weeks tracked</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-8 py-6 border-b border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-x-12 gap-y-3">
            <div className="flex justify-between border-b border-slate-50 pb-2">
              ...
              <span className="text-sm font-semibold text-slate-800">{formatCurrency(ytdIncome)}</span>
            </div>
            <div className="flex justify-between border-b border-slate-50 pb-2">
            ...
            <div className="flex justify-between border-b border-slate-50 pb-2">
              <span className="text-sm text-slate-600 font-semibold">Net Profit (Schedule C)</span>
              <span className={`text-sm font-bold ${netProfit >= 0 ? "text-emerald-600" : "text-red-500"}`}>
                {formatCurrency(netProfit)}
              </span>
            </div>
            <div className="flex justify-between border-b border-slate-50 pb-2">
              <span className="text-sm text-slate-600">Owner Draws</span>
              <span className="text-sm font-semibold text-slate-800">{formatCurrency(ytdOwnerDraws)}</span>
            </div>
            <div className="flex justify-between border-b border-slate-50 pb-2">
              <span className="text-sm text-slate-600">Personal Expenses</span>
              <span className="text-sm font-semibold text-slate-800">{formatCurrency(ytdPersonalExpenses)}</span>
            </div>
            <div className="flex justify-between border-b border-slate-50 pb-2">
              <span className="text-sm text-slate-600">Total Mileage</span>
              <span className="text-sm font-semibold text-slate-800">{totalMileage.toLocaleString()} miles</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-slate-600">Mileage Deduction (est. $0.70/mi)</span>
              <span className="text-sm font-semibold text-slate-800">{formatCurrency(mileageDeduction)}</span>
            </div>
          </div>
        </div>
        ...
        {/* Income Breakdown */}
        <div className="px-8 py-6 border-b border-slate-100">
          <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Income Breakdown</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-2 font-semibold text-slate-600">Source</th>
                <th className="text-right py-2 font-semibold text-slate-600">Amount</th>
                <th className="text-right py-2 font-semibold text-slate-600">% of Total</th>
              </tr>
            </thead>
            <tbody>
              {sortedIncome.map(([desc, amount]) => (
                <tr key={desc} className="border-b border-slate-50">
                  <td className="py-2 text-slate-700">{desc}</td>
                  <td className="py-2 text-right text-slate-800 font-medium">{formatCurrency(amount)}</td>
                  <td className="py-2 text-right text-slate-500">{((amount / ytdIncome) * 100).toFixed(1)}%</td>
                </tr>
              ))}
        ...
        {/* Expense Breakdown */}
        <div className="px-8 py-6 border-b border-slate-100">
          <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Business Expense Breakdown</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-2 font-semibold text-slate-600">Expense</th>
                <th className="text-right py-2 font-semibold text-slate-600">Amount</th>
                <th className="text-right py-2 font-semibold text-slate-600">% of Total</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr className="font-semibold">
                <td className="py-2 text-slate-800">Total Business Expenses</td>
        ...
        <div className="px-8 py-6">
          <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Weekly Detail</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-2 font-semibold text-slate-600">Week</th>
                <th className="text-right py-2 font-semibold text-slate-600">Income</th>
                <th className="text-right py-2 font-semibold text-slate-600">Expenses</th>
                <th className="text-right py-2 font-semibold text-slate-600">Net</th>
                <th className="text-right py-2 font-semibold text-slate-600">Miles</th>
              </tr>
            </thead>
            <tbody>
              {sortedEntries.map((entry) => {
                ...
                return (
                  <tr key={entry.id} className="border-b border-slate-50">
                    <td className="py-1.5 text-slate-600">{start} - {end}</td>
                    ...
                    <td className="py-1.5 text-right text-slate-500">{entry.mileage > 0 ? entry.mileage.toLocaleString() : "-"}</td>
                  </tr>
                );
              })}
            </tbody>
